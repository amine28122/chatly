    ...
    <nav className="border-b border-zinc-800/80 bg-zinc-950/90 backdrop-blur-xl sticky top-0 z-40 font-['Plus_Jakarta_Sans',sans-serif]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        >
          ...
          <div>
            ...
            <span className="text-[11px] text-zinc-400 block -mt-0.5 font-medium">
              {isClient ? `Workspace: ${user.companyName || user.name}` : (user?.role === 'viewer' ? 'Read-only analytics & bot access' : 'AI assistant management & customization platform')}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <div className="flex items-center gap-3">
              {user?.role === 'admin' && (activeView === 'dashboard' || activeView === 'bot_editor') && (
                <button
